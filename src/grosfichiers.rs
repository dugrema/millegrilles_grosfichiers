...
// async fn commande_sauvegarder_cle<M>(middleware: &M, m: MessageValideAction, gestionnaire_ca: &GestionnaireGrosFichiers)
//     -> Result<Option<MessageMilleGrille>, Box<dyn Error>>
...
//         Some(cle) => cle.as_str(),
//         None => {
//             let message = format!("grosfichiers.commande_sauvegarder_cle: Erreur validation - commande sauvegarder cles ne contient pas la cle CA : {:?}", commande);
//             warn!("{}", message);
...
//         Ok(t) => t,
//         Err(e) => Err(format!("grosfichiers.transaction_cle Erreur conversion transaction : {:?}", e))?
//     };
